...
function execute(url) {
    ...
    if (response.ok) {
        ...
        // Ưu tiên tìm theo cấu trúc HTML mới của trang
        let el = doc.select(".img-wrap img");
        
        // Dự phòng cho cấu trúc cũ
        if (el.size() === 0) {
            el = doc.select(".content div img.lazy");
        }
        if (el.size() === 0) {
        ...
        for (let i = 0; i < el.size(); i++) {
            ...
            // Thêm data-src của HTML mới, kết hợp phương án dự phòng cũ
            let imgUrl = e.attr("data-src");
